...
create-stack: 
	...
	aws s3 cp deploy.zip s3://${BUCKET}/src/BrownBotFunction.zip
	aws cloudformation create-stack --stack-name LambdaBrownBot --template-body file://cloud.yaml --parameters ParameterKey=BucketName,ParameterValue=${BUCKET} --capabilities CAPABILITY_IAM